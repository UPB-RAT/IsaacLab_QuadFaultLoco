...
class CustomManagerBasedRLEnv(CustomManagerBasedEnv, gym.Env):
    is_vector_env: ClassVar[bool] = True 
    metadata: ClassVar[dict[str, Any]] = {
        "render_modes": [None, "human", "rgb_array"],
        "isaac_sim_version": get_version(),
    }
    cfg: CustomManagerBasedRLEnvCfg

    def __init__(self, cfg: CustomManagerBasedRLEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        for _ in range(self.cfg.decimation):
            ...
            if self.cfg.run_data_collection:
                data_sample = torch.concat((self.scene["robot"].data.joint_pos, self.scene["robot"].data.joint_vel, self.scene["robot"].data.applied_torque), dim = -1)
                self.dataset.append(data_sample)
                if len(self.dataset) % 1000 == 0:
                    print(f'{len(self.dataset)} / {self.dataset_length} steps collected.')

                if len(self.dataset) == self.dataset_length:
                    dataset = torch.stack(self.dataset, dim = 0) 
                    np.save(f"quad_dynamic_dataset_{self.dataset_length}.npy", dataset.detach().cpu().numpy())
