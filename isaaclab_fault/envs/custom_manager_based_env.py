...
from .custom_manager_based_env_cfg import CustomManagerBasedEnvCfg
...
class CustomManagerBasedEnv(ManagerBasedEnv):
    def __init__(self, cfg: CustomManagerBasedEnvCfg):
        self.cfg: CustomManagerBasedEnvCfg
        cfg.validate()
        ...
        ### Debug faulty joint
        if not hasattr(self, "_fault_marker"):
            cfg = RAY_CASTER_MARKER_CFG.replace(prim_path="/Visuals/faulty_joint")
            cfg.markers["hit"].radius = 0.05  # larger dot
            self._fault_marker = VisualizationMarkers(
                cfg
            )
        if hasattr(self, "_fault_marker"):
            self._fault_marker.set_visibility(True)
