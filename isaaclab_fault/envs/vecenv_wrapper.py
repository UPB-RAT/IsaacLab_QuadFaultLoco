...
class CustomRslRlVecEnvWrapper(VecEnv):
    ...
    def get_observations(self) -> tuple[torch.Tensor, dict]:
        ...
        if self.env.unwrapped.cfg.priv_motors_info:
            motors_strength = self.env.unwrapped.scene["robot"].motors_strength
            faulty_joint_idx = self.env.unwrapped.scene["robot"].faulty_joint_idx
            applied_torque = self.env.unwrapped.scene["robot"].data.applied_torque
            obs_dict["critic"] = torch.cat([obs_dict["critic"],  # t + 1
                                        motors_strength, # t
                                        faulty_joint_idx, # t
                                        applied_torque], dim = 1) # t       
    ...
    def step(self, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        extras["episode_duration"] = current_ep_duration
